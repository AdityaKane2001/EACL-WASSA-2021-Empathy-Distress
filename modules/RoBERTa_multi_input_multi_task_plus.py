...
class RoBERTa_multi_input_multi_task_plus():
    ...
    def __init__(self, task="empathy", activation="relu", kr_rate=0.001, score_loss="mse", binary_loss="binary_crossentropy", multiclass_loss="sparse_categorical_crossentropy", cpkt="trial"):
        ...
        if self.task == "empathy":
            self.nrc_features  = ['fear', 'joy', 'sadness', 'valence', 'arousal', 'dominance']
            self.empath_features = ['domestic_work', 'death', 'home', 'sadness', 'suffering', 'help', 'party', 'family', 'shame', 'celebration', 'leisure', 'negative_emotion', 'air_travel', 'violence', 'pain']
            
        elif self.task == "distress":
            self.nrc_features  = ['anger', 'disgust', 'fear', 'sadness', 'arousal', 'dominance']
            self.empath_features = ['suffering',  'death',  'torment',  'hate',  'negative_emotion',  'sadness',  'aggression',  'fight',  'help',  'pain',  'kill',  'horror',  'violence',  'war', 'ugliness']
        ...
        self.nrc_scaler = StandardScaler()
        self.empath_scaler = StandardScaler()
    ...
    def prepare_input(self, pre, utils_obj, df, maxlen=200, padding_type='post', truncating_type='post', mode="train"):
        ...
        nrc_score = utils_obj.get_essay_nrc_scores(essay, self.nrc_features, normalize=False)
        empath_score = df[self.empath_features].values
        ...
        if mode == "train":
            ...
            nrc_score = self.nrc_scaler.fit_transform(nrc_score)
            empath_score = self.empath_scaler.fit_transform(empath_score)
            ...
            nrc_score = self.nrc_scaler.transform(nrc_score)
            empath_score = self.empath_scaler.transform(empath_score)
            iri_score = self.iri_scaler.transform(iri_score)
            personality_score = self.personality_scaler.transform(personality_score)

        nrc_score = np.reshape(nrc_score, (len(nrc_score), len(nrc_score[0])))
        empath_score = np.reshape(empath_score, (len(empath_score), len(empath_score[0])))
    ...
    def build(self, embedding_matrix, input_length=100):
        ...
        # NRC Input
        nrc_input = Input(shape=(len(self.nrc_features),), name="nrc_input")
        nrc_dense = Dense(8, activation=self.activation, kernel_initializer=self.kr_initializer)(nrc_input)

        # Empath Input
        empath_input = Input(shape=(len(self.empath_features),), name="empath_input")
        empath_dense = Dense(8, activation=self.activation, kernel_initializer=self.kr_initializer)(empath_input)
        
        x5 = concatenate([nrc_dense, empath_dense])
        x5 = Dense(32, activation=self.activation, kernel_initializer=self.kr_initializer, kernel_regularizer=l2(self.kr_rate))(x5)
